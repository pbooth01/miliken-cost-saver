...
export class SavingsCalculatorPageComponent implements OnInit {
  ...
  PowerProSavings: number = 0;
  VictoryTapeSavings: number = 0;
  PowerTapeSavings: number = 0;

  PowerProSavingsPercentage: number = 0;
  VictoryTapeSavingsPercentage: number = 0;
  PowerTapeSavingsPercentage: number = 0;

  constructor(private formBuilder: FormBuilder, private costCalcService: CostCalculationService) { 
  ...
  calculateSavings = () => {
    ...
    let competingProductCost = this.costCalcService.calculateCompetingProductCost(
      this.productForm.value.TufSkinCases, this.productForm.value.HLPadCases,
      this.productForm.value.PreWrapCases, this.productForm.value.LCWhiteTapeCases,
      this.productForm.value.HCWhiteTapeCases, this.productForm.value.STECases,
      this.productForm.value.CohesiveCases
    );
